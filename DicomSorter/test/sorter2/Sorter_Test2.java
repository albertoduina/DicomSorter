...
public class Sorter_Test2 {
	...
	public void testCopyDirectory() {
		...
		if (dir.exists()) {
			success1 = sorterUtils2.deleteDirectoryWithFiles(dir);
		} else {
			success1 = true;
		}
		assertTrue(success1);
		...
		boolean success2 = sorterUtils2.copyDirectoryWithFiles(src, dst);
		assertTrue(success2);
	}
	...
	public void testCountFiles() {
		...
		int nfiles = sorterUtils2.countFiles(filePath);
	...
	public void testListFiles() {
		String filePath = "./Dati/Work";
		ArrayList<String> list = sorterUtils2.listFiles(filePath);
	...
	public void testReadDicomParameterOk() {
		...
		String parameterValue = sorterUtils2.readDicomParameter(imp,
				DICOM_PATIENT_NAME);
	...
	public void testFilterChar() {
		...
		String out1 = sorterUtils2.filterChar(in1);
	...
	public void testReadDicomParameterMissingAttribute() {
		...
		String parameterValue = sorterUtils2.readDicomParameter(imp, INEXISTENT);
	...
	public void testMoveFile() {
		...
		boolean result = sorterUtils2.moveFile(sorgente, destinazione);
		assertTrue(result);

		sorgente = "./Dati/Work/Uno/blob.dll";
		boolean result1 = sorterUtils2.moveFile(sorgente, destinazione);
		assertTrue(result1);
	}
	...
	public void testdeleteDirectoryWithFiles() {
		...
		boolean result1 = sorterUtils2.createDirectory(name);
		...
		boolean result = sorterUtils2.deleteDirectoryWithFiles(dir1);
		assertTrue(result);
	}
	...
	public void testdeleteDirectoryEmpty() {
		...
		boolean result = sorterUtils2.deleteDirectoryEmpty(dir);
		assertTrue(result);
	}
	...
	public void testRenameFileProgressive() {
		String newName = sorterUtils2.renameFileProgressive("1234", 3);
		// System.out.printf("newName=" + newName);
		assertEquals("1234(3)", newName);
		newName = sorterUtils2.renameFileProgressive("1234.xxx", 2);
		assertEquals("1234(2).xxx", newName);
		newName = sorterUtils2.renameFileProgressive("1234(2).xxx", 5);
	...
	public void testCopyFileInexistent() {
		...
		if (dir.exists()) {
			success = sorterUtils2.deleteDirectoryWithFiles(dir);
		} else {
			success = true;
		}
		assertTrue(success);
		...
		boolean success1 = sorterUtils2.createDirectory(nameDir);
		assertTrue(success1);
		File src = new File("./Dati/Work/Uno/Due/iw2ayv.aaa");
		File dst = new File( "./Dati/Work/Moved");
		boolean success2 = sorterUtils2.copyFile(src, dst);
		assertFalse(success2);
		MyLog.waitHere("success2= "+success2);

		boolean success3 = false;
		if (dir.exists()) {
			success3 = sorterUtils2.deleteDirectoryWithFiles(dir);
		} else {
			success3 = true;
		}
		assertTrue(success3);
	...
	public void testCreateDirectory() {
		String name = "./Dati/Work/Uno/Due/Tre/Quattro/Cinque";
		boolean result = sorterUtils2.createDirectory(name);
		assertTrue(result);
		// testo che mi dia true anche se esiste gi�
		result = sorterUtils2.createDirectory(name);
		assertTrue(result);
		MyLog.waitHere();
		// ripulisco la zona
		name = "./Dati/Work/Uno/Due/Tre";
		File dir = new File(name);
		result = sorterUtils2.deleteDirectoryEmpty(dir);
		assertTrue(result);
	}
